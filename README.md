...
You need Go and MariaDB and have to have the DB set up. Create a DB and user, set the env vars `$ZENODB_NAME`, `$ZENODB_USER`, `$ZENODB_PASS`, and for remote connections also `$ZENODB_HOST`, and then import the schema from the repo you clone with `git clone https://github.com/fsncps/zeno`.
